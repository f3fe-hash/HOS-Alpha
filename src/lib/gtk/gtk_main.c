...
void initgtk()
{
    gtk.apps = calloc(MAX_APPS, sizeof(GUIApp_t));
    gtk.num_apps = 0;
...
static void on_apps_button_clicked(GtkWidget *button, gpointer user_data)
{
    ...
    // Set button size to match image size (48x48)
    gtk_widget_set_size_request(terminal_button, 32, 48);
    gtk_widget_set_hexpand(terminal_button, FALSE);
    gtk_widget_set_vexpand(terminal_button, FALSE);
    gtk_widget_set_size_request(files_button, 32, 48);
    gtk_widget_set_hexpand(files_button, FALSE);
    gtk_widget_set_vexpand(files_button, FALSE);

    // Remove button relief (flat appearance)
    gtk_button_set_relief(GTK_BUTTON(terminal_button), GTK_RELIEF_NONE);
    gtk_button_set_relief(GTK_BUTTON(files_button), GTK_RELIEF_NONE);

    // Optional: same CSS class to style like the gear icon
...
{
    GUIApp_t *gapp = (GUIApp_t *)user_data;

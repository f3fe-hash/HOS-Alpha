...
int ping(in_addr_t ip_address, int packet_size, int packets, int timeout)
{
    ...
    char* packet = mp_alloc(mpool_, packet_size);
    if (!packet)
    {
    ...
    for (int i = 0; i < packets; i++)
    {
        ...
        gettimeofday(&start_time, NULL); // FIX: added timing
        ...
        // Record end time (regardless of success to avoid undefined use)
        gettimeofday(&end_time, NULL); // FIX: added timing
        ...
        double elapsed_ms = (end_time.tv_sec - start_time.tv_sec) * 1000.0 +
                            (end_time.tv_usec - start_time.tv_usec) / 1000.0; // FIX: timing
        ...
        if (icmp_header->type == ICMP_ECHOREPLY)
        {
            ...
            printf("Reply from %s: bytes=%zd time=%.3f ms TTL=%d\n",
                   ...
                   elapsed_ms, // FIX: use measured time
                   ip_header->ttl);
        }
        else
        {

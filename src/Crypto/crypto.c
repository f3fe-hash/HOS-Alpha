...
#ifdef USE_FOOTPRINT
FOOTPRINT_TYPE footprint[FOOTPRINT_TYPE_SIZE_BYTES];
#endif
...
{
    byte *hash = malloc(SHA_DIGEST_LENGTH);
    if (!hash)
        return NULL;
...
{
    byte *hash = malloc(SHA256_DIGEST_LENGTH);
    if (!hash)
        return NULL;
...
{
    byte *hash = malloc(SHA512_DIGEST_LENGTH);
    if (!hash)
        return NULL;
...
byte *aes_encrypt(const byte *data, size_t len, AESkey_t key, int key_size, int *out_len)
{
    ...
    byte *out = malloc(len + AES_BLOCK_SIZE); // Padded output
    if (!out)
    {
